...
| **Insurance: paycheck-deducted flag** | New `bool IsPaycheckDeducted` on `FinancialProfileInsurancePolicy` (default false). When true, the premium is EXCLUDED from `CashFlowSummary.TotalMonthlyOutflows` because the salary's `MonthlyNetAmount` already nets it out (FEHB, FEDVIP, FEGLI for fed; employer pre-tax health for civilians). Surfaced separately in `OutflowSection.PaycheckDeductedInsurance` (informational, parallel to savings allotments). AI prompt annotates each paycheck-deducted policy as `[paycheck-deducted — already in net pay]` so the analyst can suggest plan changes / cheaper coverage / VA primary-care substitution without believing the user is paying twice. | Added 2026-06-03. Health and dental premiums for federal employees come off gross pay before take-home. With cash-flow basis defaulted to Net (P2), counting them again as outflows double-counts. Setting to $0 is misleading to the AI; deleting the policy loses cost visibility. The toggle gives the AI full premium visibility while keeping cash-flow math honest. Migration `20260603121324_Wave14_InsurancePaycheckDeducted`. |
...
### Phase 3 — Recurring Detection + Anomaly Alerts 📋 / ✅ P3A shipped 2026-06-03

**P3A (shipped 2026-06-03)** — heuristic recurring + anomaly + alerts foundation:
- `HeuristicRecurringDetector` clusters non-Plaid `CashTransaction` rows by normalized merchant + sign + ±5% amount tolerance + median-gap cadence inference (Weekly/Biweekly/SemiMonthly/Monthly/Annual) over the last 120 days. Requires ≥3 occurrences; ≥4 → `Mature`, else `EarlyDetection`. Internal transfers excluded via the existing 3-layer filter.
- `AnomalyDetectionService` computes per-Plaid-primary 6-month rolling median + IQR; flags transactions where `|amount − median| / IQR > 2`. Suppresses categories with fewer than 6 transactions in the window. R-7 / Excel-style quartile interpolation; medians + IQR persisted on the `SpendingAnomaly` row so the dashboard doesn't recompute.
- `SpendingAlertService` emits `Alert` rows for active anomalies — High severity at ≥4× IQR, Medium otherwise. 24h dedup via JSON `AlertKey` (matches `CryptoAlertService` pattern). New `AlertCategory.Spending` enum value.
- `SpendingRollupJob` chains the three services after rollups: rollups → heuristic detect → anomaly detect → alert emit.
- 4 new endpoints: `GET /api/Spending/recurring?direction=&isActive=`, `POST /api/Spending/recurring/{id}/dismiss`, `GET /api/Spending/anomalies?dismissed=`, `POST /api/Spending/anomalies/{id}/dismiss`.
- Frontend: `RecurringStreamsPanel` (Outflow/Inflow tabs, Plaid/Heuristic source chips, `Early` badge, dismiss action) + `AnomalyAlertsCard` (High/Medium severity, deviation multiple, dismiss). Both wired into `SpendingView`.
